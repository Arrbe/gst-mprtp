...
struct _Private{
  ...
  GstClockTime        qdelay_cong_treshold;
...
#define _qdelay_cong_th(this)         _priv(this)->qdelay_cong_treshold
#define _dist_keep_th(this)           _priv(this)->qdelay_distortion_keep_th
#define _qdelay_dist_probe_th(this)   _priv(this)->qdelay_distortion_probe_th
#define _qdelay_dist_keep_th(this)    _priv(this)->qdelay_distortion_probe_th
...
//  }
  ...
  if(!_fcongestion(this)){
    target_rate = MIN(_TR(this) * .85, _TR_t1(this));
  }else{
    target_rate = MIN(_TR(this) * .85, _TR(this) * _rdc_target_fac(this));
  }
done:
  return target_rate;
}
    ...
    FBRASubController *this)
{
  ...
  //_priv(this)->stage_changed < _now(this) - 5 * GST_SECOND
  if(_qdelay_median(this) + _qdelay_cong_th(this) < _qdelay_actual(this)){
    congestion = TRUE;
  }else if(_UF(this) < _discard_cong_th(this)){
    congestion = TRUE;
  }

  if(_qdelay_median(this) + _qdelay_dist_keep_th(this) < _qdelay_actual(this)){
    distortion = TRUE;
  }else if(_UF(this) < 1.){
    distortion = TRUE;
  }
...
//  target_rate *= 1.-_q4(this);
  if(_fdistortion(this) || _bdistortion(this)){
  ...
  if(_qdelay_median(this) + _qdelay_dist_probe_th(this) < _qdelay_actual(this)){
    goto done;
  }
